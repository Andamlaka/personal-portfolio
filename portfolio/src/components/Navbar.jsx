    ...
    <nav className="fixed top-0 left-0 w-full z-50 backdrop-blur-md shadow-md bg-white dark:bg-primary text-primary dark:text-white">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center space-x-4 lg:space-x-6 ml-auto">
          <NavLink to="/" className={linkClass}>Home page</NavLink>
          <NavLink to="/about" className={linkClass}>About me</NavLink>
          <NavLink to="/portfolio" className={linkClass}>Portfolio</NavLink>
          <NavLink to="/blog" className={linkClass}>Blog</NavLink>

          <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-blue-500">
            <FaLinkedin />
          </a>
          <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-blue-500">
            <FaBehance />
          </a>
          <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-red-600">
            <FaYoutube />
          </a>

          <button className="bg-primary text-sm sm:text-base leading-[45px] dark:bg-white font-semibold font-dm  text-white
...
{menuOpen && (
  <div className="md:hidden bg-white dark:bg-primary shadow-lg border-t border-gray-200 dark:border-gray-700 cursor-pointer">
    <div className="flex flex-col px-4 py-4 space-y-4">
      {/* Links */}
      <NavLink to="/" className={linkClass} onClick={() => setMenuOpen(false)}>Home page</NavLink>
      <NavLink to="/about" className={linkClass} onClick={() => setMenuOpen(false)}>About me</NavLink>
      <NavLink to="/portfolio" className={linkClass} onClick={() => setMenuOpen(false)}>Portfolio</NavLink>
      <NavLink to="/screenshots" className={linkClass} onClick={() => setMenuOpen(false)}>Screenshots</NavLink>

      {/* Social Icons */}
      <div className="flex items-center justify-start space-x-4 mt-2">
        <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-blue-500"><FaLinkedin /></a>
        <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-blue-500"><FaBehance /></a>
        <a href="#" className="text-gray-600 text-xl dark:text-gray-300 hover:text-red-600"><FaYoutube /></a>
      </div>

      {/* Get in touch button */}
      <button
        className="w-full bg-primary text-sm sm:text-base leading-[45px] dark:bg-white font-semibold font-dm text-white
         dark:text-black px-4 sm:px-6 py-2 rounded-full mt-2 cursor-pointer"
